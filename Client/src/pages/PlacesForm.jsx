...
export default function PlacesFormPage(){
                ...
                <div className="mb-2 grid grid-cols-3 lg:grid-cols-4 gap-2 md:grid-cols-3 sm:grid-cols-2">
                    {addedPhotos.map((photo, index) =>(
                        <div key={index} className="relative h-32 flex">
                            ...
                            />
                            <div className="absolute bottom-2 right-2 bg-white p-1 rounded-2xl text-secondary">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                    <div>
                        <h4>Check in time</h4>
                        <input type="number" value={checkIn} onChange={ev => setCheckIn(ev.target.value)} placeholder="24:00" />
                    </div>
                    <div>
                        <h4>Check out time</h4>
                        <input type="number" value={checkOut} onChange={ev => setCheckOut(ev.target.value)} placeholder="24:00" />
                    </div>
                    <div>
                        <h4>Max allowed guests</h4>
                        <input type="number" value={guests} onChange={ev => setGuests(ev.target.value)} placeholder="Guests" />
                    </div>
                </div>
